...
class ObserveTaxonomiesUseCaseTest {
    ...
    fun `the catalogue arrives in the order it was published`() =
        runTest {
            val useCase = ObserveTaxonomiesUseCase(StubCataloguePort(catalogue))
    ...
    fun `a failing listener reports on errors and publishes no catalogue`() =
        runTest {
            ...
            val useCase = ObserveTaxonomiesUseCase(StubCataloguePort(failure = error))
