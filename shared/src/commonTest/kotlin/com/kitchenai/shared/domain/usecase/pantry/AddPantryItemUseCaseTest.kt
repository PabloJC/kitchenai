...
class AddPantryItemUseCaseTest {
    ...
    fun `tops up the held row when the ingredient and the unit match`() =
        runTest {
            ...
            val result = useCase(port)(kitchen, ingredientId("ing-1"), null, Quantity(50.0, unitA), null, sooner)
    ...
    fun `keeps a separate row when the unit differs because nothing is converted`() =
        runTest {
            ...
            useCase(port)(kitchen, ingredientId("ing-1"), null, Quantity(1.0, unitB), null, null)
    ...
    fun `a new holding takes the generated id and the current time`() =
        runTest {
            val result =
                useCase(FakePantryRepositoryContract())(kitchen, ingredientId("ing-1"), null, Quantity(3.0), null, null)
    ...
    fun `rejects an amount of zero without touching the port`() =
        runTest {
            ...
            val result = useCase(port)(kitchen, ingredientId("ing-1"), null, Quantity(0.0, unitA), null, null)
    ...
    fun `propagates a write failure instead of reporting the item as added`() =
        runTest {
            ...
            val result = useCase(port)(kitchen, ingredientId("ing-1"), null, Quantity(3.0), null, null)
    ...
    fun `a free-text holding is added even though the catalogue has never heard of it`() =
        runTest {
            ...
            val result = useCase(port)(kitchen, null, "the good bread", Quantity(1.0), null, null)
    ...
    fun `a free-text holding never merges with an existing one`() =
        runTest {
            ...
            useCase(port)(kitchen, null, "the good bread", Quantity(1.0), null, null)
    ...
    fun `rejects both an ingredient and free text on the same holding`() =
        runTest {
            ...
            val result = useCase(port)(kitchen, ingredientId("ing-1"), "the good bread", Quantity(1.0), null, null)
    ...
    fun `rejects both an ingredient and free text even when a mergeable holding already exists`() =
        runTest {
            ...
            val result =
                useCase(port)(kitchen, ingredientId("ing-1"), "the good bread", Quantity(50.0, unitA), null, null)
