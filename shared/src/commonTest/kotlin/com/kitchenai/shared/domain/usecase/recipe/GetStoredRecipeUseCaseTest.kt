...
class GetStoredRecipeUseCaseTest {
    private val stored = recipe(id = "recipe-1")

    @Test
    fun `finds a recipe from the last generation by id`() =
        runTest {
            val useCase = GetStoredRecipeUseCase(FakeRecipeRepositoryContract(stored = listOf(stored)))
    ...
    fun `a recipe from no generation is absent rather than an error`() =
        runTest {
            val useCase = GetStoredRecipeUseCase(FakeRecipeRepositoryContract())
    ...
    fun `a failing local read is reported`() =
        runTest {
            val useCase = GetStoredRecipeUseCase(FakeRecipeRepositoryContract(readError = AppError.Unknown()))
