...
# ------------------------------------------------------------------ #
# Enlazar el tablero al repositorio. Es un proyecto de usuario, así que sin
# esto no aparece en la pestaña Projects del repo ni en la barra lateral de
# las issues. El workflow funciona igual —encuentra el tablero por número y
# dueño—, pero trabajar sin el enlace es incómodo.
gh project link "$PROJECT_NUMBER" --owner "$OWNER" --repo "$REPO" >/dev/null 2>&1 \
    && printf '%s✓%s Tablero enlazado a %s\n' "$GREEN" "$OFF" "$REPO" \
    || printf '%s·%s El tablero ya estaba enlazado a %s\n' "$YELLOW" "$OFF" "$REPO"
